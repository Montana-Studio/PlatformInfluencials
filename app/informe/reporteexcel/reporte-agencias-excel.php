<?php
		...
		$tituloPaginas="Metricas para ".$descripcion_rrss." - Campaña : ".$campana;
			...
			if($descripcion_rrss=='facebook'){
					...
					$objRedSocialSheet->setCellValue('A2', $tituloPaginas)
					            		...
					            		->setCellValue('D'.$i,  $likes)
							            ->setCellValue('E'.$i,  $comments)
					        		    ->setCellValue('F'.$i,  $shares)
			...
			if($descripcion_rrss=='twitter'){
					...
					$objRedSocialSheet->setCellValue('A2', $tituloPaginas)
					            		...
					            		->setCellValue('C'.$i,  $url)
							            ->setCellValue('D'.$i,  $favorites)
					        		    ->setCellValue('E'.$i,  $retweets)
					            		->setCellValue('F'.$i,  $followers)
					            		->setCellValue('G'.$i,  number_format($reach,3));
			}
			...
			if($descripcion_rrss=='youtube'){
					...
					$objRedSocialSheet->setCellValue('A2', $tituloPaginas)
					            		...
					            		->setCellValue('C'.$i,  $url)
					            		->setCellValue('D'.$i,  $reproducciones)
					            		->setCellValue('E'.$i,  number_format($reach,3));
			}
		}
	...
	if($num_row_datos_campana > 0 ){
		...
		$titulosColumnasInfluenciadores2 = array('NOMBRE', 'CUENTA', 'URL', 'LIKES', 'COMMENTS', 'SHARES', 'FOLLOWERS', 'FAVORITES', 'RETWEET', 'REPRODUCCIONES', 'REACH');
        ...
        $objRedSocialSheet = $objPHPExcel->createSheet(1)
							...
							$objPHPExcel->setActiveSheetIndex(1)
									 	->setCellValue('A3',  $titulosColumnasInfluenciadores2[0])
							            ->setCellValue('B3',  $titulosColumnasInfluenciadores2[1])
					        		    
					            		->setCellValue('C3',  $titulosColumnasInfluenciadores2[2])
					            		->setCellValue('D3',  $titulosColumnasInfluenciadores2[3])
					            		->setCellValue('E3',  $titulosColumnasInfluenciadores2[4])
					            		->setCellValue('F3',  $titulosColumnasInfluenciadores2[5])
					            		->setCellValue('G3',  $titulosColumnasInfluenciadores2[6])
					            		->setCellValue('H3',  $titulosColumnasInfluenciadores2[10])
        ...
        $objRedSocialSheet = $objPHPExcel->createSheet(3)
				            ...
				            $objPHPExcel->setActiveSheetIndex(3)
					            		...
					            		->setCellValue('C3',  $titulosColumnasInfluenciadores2[2])
					            		->setCellValue('D3',  $titulosColumnasInfluenciadores2[7])
					            		->setCellValue('E3',  $titulosColumnasInfluenciadores2[8])
					            		->setCellValue('F3',  $titulosColumnasInfluenciadores2[6])
					            		->setCellValue('G3',  $titulosColumnasInfluenciadores2[10])
        ...
        $objRedSocialSheet = $objPHPExcel->createSheet(4)
					        ...
					        $objPHPExcel->setActiveSheetIndex(4)
					            		...
					            		->setCellValue('C3',  $titulosColumnasInfluenciadores2[2])
					            		->setCellValue('D3',  $titulosColumnasInfluenciadores2[9])
					            		->setCellValue('E3',  $titulosColumnasInfluenciadores2[10])
		...
		do{
			...
			if($reach_facebook>0){
				$objPHPExcel->setActiveSheetIndex(1)
        		    ->mergeCells('A'.$facebook_posts.':G'.$facebook_posts)
        		    ->setCellValue('A'.$facebook_posts, 'TOTAL')
        		    ->setCellValue('H'.$facebook_posts, $reach_facebook);
        		}else{
        			$objPHPExcel->setActiveSheetIndex(1)
        		    ->mergeCells('A1:H10')
        		    ->setCellValue('A1', 'NO SE REGISTRARON URLS PARA ESTA CAMPAÑA');
        		}
			
			if($reach_instagram>0){
				$objPHPExcel->setActiveSheetIndex(2)
        		    ->mergeCells('A'.$instagram_posts.':G'.$instagram_posts)
        		    ->setCellValue('A'.$instagram_posts, 'TOTAL')
        		    ->setCellValue('H'.$instagram_posts, $reach_instagram);
			}else{
				$objPHPExcel->setActiveSheetIndex(2)
        		    ->mergeCells('A1:H10')
        		    ->setCellValue('A1', 'NO SE REGISTRARON URLS PARA ESTA CAMPAÑA');
        	...
        	if($reach_twitter>0){
        		$objPHPExcel->setActiveSheetIndex(3)
        		    ->mergeCells('A'.$twitter_posts.':F'.$twitter_posts)
        		    ->setCellValue('F'.$twitter_posts, 'TOTAL')
        		    ->setCellValue('G'.$twitter_posts, $reach_twitter);
    		}else{
    			$objPHPExcel->setActiveSheetIndex(3)
        		    ->mergeCells('A1:H10')
        		    ->setCellValue('A1', 'NO SE REGISTRARON URLS PARA ESTA CAMPAÑA');
        	...
        	if($reproducciones_youtube>0){
        		$objPHPExcel->setActiveSheetIndex(4)
        		    ->mergeCells('A'.$youtube_posts.':D'.$youtube_posts)
        		    ->setCellValue('A'.$youtube_posts, 'TOTAL')
        		    ->setCellValue('E'.$youtube_posts, $reproducciones_youtube);
        		}else{
        			$objPHPExcel->setActiveSheetIndex(4)
        		    ->mergeCells('A1:H10')
        		    ->setCellValue('A1', 'NO SE REGISTRARON URLS PARA ESTA CAMPAÑA');
        ...
        //ELimino Redes no incluídas en campana
