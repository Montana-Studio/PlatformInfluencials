...
function html_facebook($mysqli, $persona_id, $red_social){
	...
	return $session_facebook."!".$alcance_facebook;
}
...
function html_instagram($mysqli, $persona_id, $red_social){
	...
	return $session_instagram."!".$alcance_instagram;
}
...
function html_twitter($mysqli, $persona_id, $red_social){
	...
	return $session_twitter."!".$alcance_twitter;
}
...
function html_youtube($mysqli, $persona_id, $red_social){
	...
	return $session_youtube."!".$alcance_youtube;
}
...
function core_facebook($red_social, $mysqli, $persona_id){
    ...
    $facebookAppId = FACEBOOK_APP_ID;
	    if($num_row6>0){
	      ...
	      return $session_facebook.$suma_red_social;
    }
}
...
function core_instagram($red_social, $mysqli, $persona_id){
	  ...
	  $num_row3= mysqli_num_rows($result3);
	  	if($num_row3>0){
		    ...
		    return $session_instagram.$suma_red_social;
    ...
    $settings = array(
      ...
      'consumer_secret' => TWITTER_CONSUMER_SECRET
		...
		if($num_row4>0){
	      ...
	      return $session_twitter.$suma_red_social;
...
function core_youtube($red_social, $mysqli, $persona_id){
    ...
    $num_row5=mysqli_num_rows($result5);
	    if($num_row5>0){
	      ...
	      return $session_youtube.$suma_red_social;
...
function muestra_followers($persona_id){
	$mysqli = mysqli_connect(LOCAL,USER,PASS,BD) or die("Error " . mysqli_error($link)); 
	$mysqli->set_charset('utf8_bin');

	$query_redes_persona="SELECT DISTINCT descripcion_rrss FROM rrss WHERE persona_id='".$persona_id."' ORDER BY descripcion_rrss";
	$result_redes_persona=mysqli_query($mysqli,$query_redes_persona)or die (mysqli_error());
	$row_redes_persona= mysqli_fetch_array($result_redes_persona, MYSQLI_BOTH);
	 
	if(requiere_actualizar($persona_id)==0){
        do{
			if($row_redes_persona[0]=='facebook'){
				$html_facebook=html_facebook($mysqli, $persona_id, $row_redes_persona[0]);
			}
			if($row_redes_persona[0]=='instagram'){
				$html_instagram=html_instagram($mysqli, $persona_id, $row_redes_persona[0]);
			}
			if($row_redes_persona[0]=='twitter'){
				$html_twitter=html_twitter($mysqli, $persona_id, $row_redes_persona[0]);
			}
			if($row_redes_persona[0]=='youtube'){
				$html_youtube=html_youtube($mysqli, $persona_id, $row_redes_persona[0]);
			}
		...
		$query_no_actualizado="SELECT DISTINCT * FROM session_table WHERE working_hours='1' AND renovado='0' AND persona_id='".$persona_id."'";
