...
for($i=0;$i<count($redes_sociales);$i++){
	$_SESSION['holi'].= '<ul class="redes-metrics"><span class="tit-red red-'.$redes_sociales[$i].'" title="'.$redes_sociales[$i].'"></span>';
	...
	if($redes_sociales[$i]=='facebook'){
		...
		if($num_row_facebook_asociado_campana>0){
			...
			$_SESSION['holi'].= '<div class="data"><ul>';
			...
			for($k=0;$k<count($paginas_asociadas)-1;$k++){
				...
				do{
		   		...
		   		/*$_SESSION['holi'].= '<li><span>Reach ['.$links_user_url1->name.']</span><span>'.$reach_facebook.'</span></li>';
		        $_SESSION['holi'].= '<li><span>Likes '.$cuenta_likes_facebook.'</span></li>';
		        $_SESSION['holi'].= '<li><span>Comments '.$cuenta_comments_facebook.'</span></li>';
		        $_SESSION['holi'].= '<li><span>Shares '.$cuenta_shares_facebook.'</span></li>';
		        $_SESSION['holi'].= '<li><span>Followers '.$followers_facebook.'</span></li>';
				$_SESSION['holi'].= '</ul></div>';*/
				$reach_total_facebook += $reach_facebook;
			}
	   			$_SESSION['holi'].= '<li><span>Reach</span><span>'.number_format($reach_total_facebook,2,".",",").'</span></li>';
	   			$_SESSION['holi'].= '</ul></div>';
		}else{
			$_SESSION['holi'].= '<div class="data"><ul>';
			$_SESSION['holi'].= '<li><span>Reach</span><span>0</span></li>';
	   		$_SESSION['holi'].= '</ul></div>';
		}
		...
		if($num_row_instagram_asociado_campana>0){
			...
			for($l=0;$l<count($paginas_asociadas)-1;$l++){
		        ...
		        $links_user_url= json_decode($json_user);
			   	...
			   	/*$_SESSION['holi'].= '<li> Likes :'.$cuenta_likes_instagram.'</li>';
				$_SESSION['holi'].= '<li> Comments :'.$cuenta_comments_instagram.'</li>';
				$_SESSION['holi'].= '<li> Followers :'.$followers_instagram.'</li>';
				$_SESSION['holi'].= '<li> Reach :'.$reach_instagram.'</li>';*/				
			}
			$_SESSION['holi'].= '<li><span>Reach</span><span>'.number_format($reach_total_instagram,2,".",",").'</span></li>';
			$_SESSION['holi'].= '</ul></div>';
		}else{
			$_SESSION['holi'].= '<div class="data"><ul>';
			$_SESSION['holi'].= '<li><span>Reach</span><span>0</span></li>';
		...
		if($num_row_twitter_asociado_campana>0){
			...
			for($m=0;$m<count($paginas_asociadas)-1;$m++){
				...
				do{
					...
					//$_SESSION['holi'].= TWITTER_CONSUMER_KEY;
					$settings = array(
				...
				//$_SESSION['holi'].= '<li><span>Likes</span><span>'.$twitter_favorite_total.'</span></li>';
				//$_SESSION['holi'].= '<li><span>Retweets</span><span>'.$twitter_retweet_total.'</span></li>';
				//$formatted_reach_twitter = ($twitter_retweet_total+$twitter_favorite_total)/$twitter_followers_total;*/
			}
				$_SESSION['holi'].= '<li><span>Reach</span><span>'.number_format($reach_total_twitter,2,".",",").'</span></li>';
				$_SESSION['holi'].= '</ul></div>';
		}else{
			$_SESSION['holi'].= '<div class="data"><ul>';
			$_SESSION['holi'].= '<li><span>Reach</span><span>0</span></li>';
	...
	if($redes_sociales[$i]=='youtube'){
		...
		if($num_row_youtube_asociado_campana>0){
	    	...
	    	//$_SESSION['holi'].= '<li> Views :'.$youtube_video_views_total.'</li>';
            $_SESSION['holi'].= '<li><span>Reach</span><span>'.$youtube_video_views_total.'%</span></li>';
			$_SESSION['holi'].= '<li> Likes :'.$youtube_video_likes_total.'</li>';
			$_SESSION['holi'].= '<li> Comments :'.$youtube_video_comments_total.'</li>';
			$_SESSION['holi'].= '<li> Reach :'.$formatted_reach_youtube.'%</li>';
			$_SESSION['holi'].= '</ul></div>';
		}
	}
	$_SESSION['holi'].= '</ul>';
