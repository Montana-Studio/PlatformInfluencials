	...
	if($num_row_no_laboral>0){
		...
		if($num_row_actualizado>0){
			...
			if($row_no_actualizado>0){
				...
				//actualizo datos
				$query_actualiza_datos="UPDATE session_table SET actual_inicio_sesion='".date("Y-m-d H:i")."', renovado='1' WHERE campana_id=".$campana_id." AND persona_id='".$_SESSION['id']."'";
				...
				if($num_row_primera_vez==0){
					...
					if(date('H')>8 && date('H')<22){
						...
						//inserto datos
						$query_inserta_datos="INSERT INTO session_table (working_hours,renovado, primer_inicio_sesion, actual_inicio_sesion, campana_id, persona_id) VALUES ('1', '1', '".date("Y-m-d H:i")."' ,'".date("Y-m-d H:i")."', ".$campana_id.", ".$_SESSION['id'].")";
