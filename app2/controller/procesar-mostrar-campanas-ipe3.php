    ...
    if($num_rows_url_campana_rrss>0){
    	do{
	        ...
	        $campanas_finalizadas .= '
	                <div class="rrss" name="'.$$rrss_name.'" >
	                    <i class="pi pi-'.$$rrss_name.'"></i>
	                    <span>['.$$rrss_name.']</span>
	                    <p id="'.$rrss_id.'" name="'.$$rrss_name.'"> '.$url.'</p>
	                    <p>Reach '.number_format($row_url_campana_rrss[14],2,".",",").'</p>
	                </div>';
...
function muestra_campanas($persona_id){
    ...
    if($num_row2>0){
        ...
        if($num_row_campanas_inactivas > 0){
            $inactivas.= muestra_campanas_inactivas($row_campanas_inactivas,$result_campanas_inactivas, $mysqli);
            $acumula_campanas++;
        }
    ...
    if($acumula_campanas==0){
        	return "No ha sido solicitado a participar de campañas";
    }else{
