...
<?php 
	...
	$result_cuenta_campanas= $mysqli->query("SELECT COUNT(id) FROM campana WHERE idpersona='".$_GET['id']."'");
	$row_cuenta_campanas= mysqli_fetch_array($result_cuenta_campanas, MYSQLI_BOTH);
	$i=0;
	...
	do{
		...
		<tr>
		    ...
		    <td><button class='informe' name='".$row_campanas_habilitadas[0]."'>Ver Informe</button></td>
	  </tr>";
	  $i++;
	}while($i < $row_cuenta_campanas[0]);
	 ?>
	 </table>
